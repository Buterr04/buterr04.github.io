...
## 技术栈
- 多模态大模型 Gemini 2.5 Flash
- 向量数据库 Chroma
- Agent 框架 LangChain

## 项目流程
1. **图像识别**：使用 Gemini 2.5 Flash 多模态大模型对上传的快递包裹图像进行分析，识别破损类型和程度。
2. **文本处理**：将快递公司的赔付政策文档进行预处理，分段存储至 Chroma 向量数据库，便于后续检索。
3. **增强检索（RAG）**：基于用户上传的破损图像识别结果，构建查询向量，从 Chroma 数据库中检索相关赔付政策条款。
4. **Agent决策**：使用 LangChain 框架构建赔付决策Agent，结合识别结果和赔付条款进行决策。
5. **用户界面**：使用JS和HTML构建用户界面，允许用户快速查看赔付结果。
